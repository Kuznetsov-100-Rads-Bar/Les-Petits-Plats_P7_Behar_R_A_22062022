...
class Appliances {
  ...
  displayAppliancesList = (filter) => {
    ...
    if (filter) {
      if (!appliancesList.find((appliance) => appliance.toLowerCase().includes(filter.toLowerCase()))) {
        appliancesList.forEach((appliance) => this.createItem(appliance));
      } else {
        appliancesList.filter((appliance) => appliance.toLowerCase().includes(filter.toLowerCase()))
          .forEach((appliance) => this.createItem(appliance));
      }
    } else {
      appliancesList.forEach((appliance) => this.createItem(appliance));
    }

    return 'Appliances list displayed';
  }
